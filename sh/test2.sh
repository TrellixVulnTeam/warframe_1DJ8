...
do
  ...
  rm -fr $data_dir/*
  for h in $(seq 2 $(($COLUMN-1)));
do
lll=`cat $lock_dir/f$h`
  qqq=`echo "$lll/$RXZ*$(($JX-$UX))" |bc |awk -F'.' '{print $1}'`
  touch $data_dir/$(($UX+$qqq))
  echo $h >> $data_dir/$(($UX+$qqq))
done
